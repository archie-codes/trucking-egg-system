...
//                     ) : (
//                       <>
//                         <Save className="w-4 h-4 mr-2 shrink-0 transition-transform group-hover/btn:scale-110 duration-300" />
...
const TripActionsCell = ({ trip }: { trip: TripRecord }) => {
  ...
  // ✨ ADDED: Autocomplete states for the Edit Modal
  ...
  useEffect(() => {
    ...
    // ✨ ADDED: Fetch Historical Autocomplete Suggestions
    async function fetchSuggestions() {
      if (customerSuggestions.length > 0) return;
      const res = await getTripHistorySuggestions();
      if (res.success) {
    ...
    fetchLocations();
    fetchSuggestions();
  }, [isEditOpen, phLocations.length, customerSuggestions.length]);
            ...
            <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6 custom-scrollbar">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  {/* ✨ UPDATED: Farm Name Autocomplete */}
                  <div className="space-y-1.5 sm:col-span-2">
                    <Label className="text-[11px] font-bold text-slate-500 uppercase">
                      Farm Name
                    </Label>
