...
export default function NewDailyRecordPage() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const toastId = toast.loading("Saving daily record...");
    try {
      const response = await createFarmDailyRecord(values);
      if (response.success) {
        toast.success("Daily record saved successfully!", { id: toastId });
        router.push("/egg-sales/farm-operations/daily-records");
      } else {
        toast.error(response.error || "Failed to save daily record", {
          id: toastId,
        });
      }
    } catch (error) {
    ...
    <div className="sm:h-[95vh] w-full mx-auto space-y-3 animate-in fade-in duration-300 max-w-3xl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
            Log daily mortality and egg production for a specific batch.
          </p>
        </div>
      ...
      <Card className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs">
        <CardHeader className="border-b border-slate-100 dark:border-slate-800/80 pb-4">
          ...
          <CardDescription>
            Specify the batch, mortality count, and egg collection numbers.
          </CardDescription>
        </CardHeader>
