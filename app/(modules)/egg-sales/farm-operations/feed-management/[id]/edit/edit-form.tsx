...
export default function EditFeedRecordForm({ record }: { record: FeedRecord }) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      quantityBags: record.quantityBags,
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const toastId = toast.loading("Updating feed record...");
    try {
      const response = await updateFarmFeedConsumption(record.id, values);
      if (response.success) {
        toast.success("Feed record updated successfully!", { id: toastId });
        router.push("/egg-sales/farm-operations/feed-management");
      } else {
        toast.error(response.error || "Failed to update feed record", {
          id: toastId,
        });
      }
    } catch (error) {
    ...
    <div className="sm:h-[95vh] w-full mx-auto space-y-3 animate-in fade-in duration-300 max-w-3xl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
            Modify feed type, bag count, or cost information.
          </p>
        </div>
      ...
      <Card className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs">
        <CardHeader className="border-b border-slate-100 dark:border-slate-800/80 pb-4">
          ...
          <CardDescription>
            Update feed type, quantity in bags, and total cost.
          </CardDescription>
        </CardHeader>
