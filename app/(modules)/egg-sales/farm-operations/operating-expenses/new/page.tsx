...
export default function NewOperatingExpensePage() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      const response = await createFarmOperatingExpense(values);
      if (response.success) {
        toast.success("Operating expense logged successfully!", { id: toastId });
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
            Record farm overhead costs (diesel, salaries, utilities) tied to a batch.
          </p>
        </div>
