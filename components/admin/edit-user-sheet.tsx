...
export function EditUserSheet({ isOpen, onClose, user }: EditUserProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      getAdminRoleAndDept().then(({ id, role, department }) => {
        setCurrentAdminId(id);
        setCurrentAdminRole(role);
        setCurrentAdminDept(department);
      });
    }
  }, [isOpen]);
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Disabled fields are omitted from FormData, so we must inject them from state
  ...
  async function executeSubmit(formData: FormData) {
    ...
    if (result.success) {
      if ("demotedSelf" in result && result.demotedSelf) {
        toast.success("Role updated. Logging you out...");
  ...
  return (
    <>
      ...
      >
        <AlertDialogContent className="sm:max-w-[400px] rounded-2xl bg-background border-border/60 z-250">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-[15px] font-semibold text-rose-600 leading-tight flex items-center gap-2">
              <ShieldAlert className="h-5 w-5" /> Are you absolutely sure?
            </AlertDialogTitle>
            <AlertDialogDescription className="mt-2 text-[13px] text-muted-foreground leading-relaxed">
              You are about to change your own account role to{" "}
              <strong>Encoder</strong>. If you save this, you will immediately
              lose administrator privileges and will not be able to access the
              admin panel anymore.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-4 flex gap-2">
            ...
            <Button
              ...
              className="flex-1 h-10 rounded-xl text-sm font-semibold bg-rose-600 hover:bg-rose-700 text-white border-0 shadow-sm transition-all"
            >
              {isSubmitting ? (
                <>
